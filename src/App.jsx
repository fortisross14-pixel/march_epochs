  ...
  return <div className="app-shell mobile-first-shell">
    ...
    <main>
      ...
      {screen==='artifacts'&&<section className="screen-panel mobile-screen"><div className="simple-screen-title"><span>PERMANENT EQUIPMENT</span><h1>Artifacts</h1><p>Artifacts now come from packs. Equip up to two Army Artifacts on each unit type; Hero Relics unlock later.</p></div><div className="content-pad"><div className="subtabs">{[['army','Army Artifacts'],['relics',features.relics?'Hero Relics':'🔒 Relics'],['store','Pack Store']].map(([id,n])=><button key={id} className={artifactTab===id?'active':''} onClick={()=>{if(id==='relics'&&!features.relics)notify('Hero Relics unlock after winning battle 2-5.');else setArtifactTab(id)}}>{n}</button>)}</div>
        {artifactTab==='army'&&<><div className="section-head"><h3>Your Army Artifacts</h3><span>{Object.values(ITEMS).filter(i=>i.slot==='unit'&&availableItemCopies(i.id)>0).length} available in inventory</span></div>{Object.values(ITEMS).filter(i=>i.slot==='unit'&&availableItemCopies(i.id)>0).length?<div className="artifact-grid list-style">{Object.values(ITEMS).filter(i=>i.slot==='unit'&&availableItemCopies(i.id)>0).map(item=>{const copies=meta.inventory[item.id]||0,available=availableItemCopies(item.id),equipped=equippedItemCount(item.id),level=meta.artifactLevels[item.id]||1,max=level>=item.maxLevel,need=max?0:ARTIFACT_COPY_THRESHOLDS[level],cost=max?0:ARTIFACT_WORKSHOP_GOLD[level];return <div className="artifact-card owned-artifact polished" key={item.id}><div className="artifact-art"><div className="artifact-level-badge">Lv {level}</div>{ASSETS.items?.[item.id]?<img src={ASSETS.items[item.id]} alt={item.name}/>:<span>🛡</span>}</div><div className="artifact-copy"><div className="artifact-title-row"><h3>{item.name}</h3><span className={`rarity-chip ${item.rarity.toLowerCase()}`}>{item.rarity}</span></div><p>{effectSummary(item,level)}</p><small>Available {available} · Total {copies}{equipped?` · ${equipped} equipped`:''}{!max?` · ${need} for next level`:''}</small><div className="artifact-actions"><button onClick={()=>setEquipPicker({itemId:item.id})}>Equip</button><button disabled={max||copies<need||meta.gold<cost} onClick={()=>evolveArtifact(item.id)}>{max?'MAX':`Evolve · ◉ ${cost}`}</button></div></div></div>})}</div>:<div className="empty-state"><b>No Army Artifacts available</b><span>Open a Supply Artifact Pack in the Pack Store. Equipped Artifacts are hidden from this inventory list until you remove them from a unit.</span><button className="primary" onClick={()=>setArtifactTab('store')}>Open Pack Store</button></div>}</>}
        {artifactTab==='relics'&&features.relics&&<><div className="section-head"><h3>Your Hero Relics</h3><span>{Object.values(ITEMS).filter(i=>i.slot==='hero'&&availableItemCopies(i.id)>0).length} available in inventory</span></div>{Object.values(ITEMS).filter(i=>i.slot==='hero'&&availableItemCopies(i.id)>0).length?<div className="artifact-grid list-style">{Object.values(ITEMS).filter(i=>i.slot==='hero'&&availableItemCopies(i.id)>0).map(item=>{const level=meta.artifactLevels[item.id]||1,copies=meta.inventory[item.id]||0,available=availableItemCopies(item.id),equipped=equippedItemCount(item.id),max=level>=item.maxLevel,need=max?0:ARTIFACT_COPY_THRESHOLDS[level],cost=max?0:ARTIFACT_WORKSHOP_GOLD[level];return <div className="artifact-card owned-artifact polished" key={item.id}><div className="artifact-art"><div className="artifact-level-badge">Lv {level}</div>{ASSETS.items?.[item.id]?<img src={ASSETS.items[item.id]} alt={item.name}/>:<span>♛</span>}</div><div className="artifact-copy"><div className="artifact-title-row"><h3>{item.name}</h3><span className={`rarity-chip ${item.rarity.toLowerCase()}`}>{item.rarity}</span></div><p>{effectSummary(item,level)}</p><small>Available {available} · Total {copies}{equipped?` · ${equipped} equipped`:''}{!max?` · ${need} for next level`:''}</small><div className="artifact-actions"><button onClick={()=>setEquipPicker({itemId:item.id})}>Equip</button><button disabled={max||copies<need||meta.gold<cost} onClick={()=>evolveArtifact(item.id)}>{max?'MAX':`Evolve · ◉ ${cost}`}</button></div></div></div>})}</div>:<div className="empty-state"><b>No Hero Relics available</b><span>Relic packs appear in the store once this feature is unlocked. Equipped Relics are hidden from this inventory list.</span></div>}</>}
    ...
    <Modal open={musterOpen} onClose={()=>setMusterOpen(false)} title={`Muster · ${startingCapacity} starting squads`} wide>
      <div className="muster-compact-layout">
        ...
        <div className="muster-eligible-panel"><div className="section-head"><h3>Eligible this run</h3><span>From unlocks + current loadout</span></div><div className="muster-pool-list compact">{availableMusterUnits().map(id=>{const u=unitStatsAtLevel(id,meta.unitLevels[id]||1),art=ASSETS.units?.[id];return <div key={id} className="muster-pool-item"><div className="mini-unit-glyph">{art?<img src={art} alt={u.name}/>:unitIcon(u)}</div><div><b>{u.name} · Lv {meta.unitLevels[id]||1}</b><span>{u.role} · ♥ {u.health} · ⚔ {u.damage} · 🛡 {u.armor}</span></div></div>})}</div><button className="primary big" onClick={launchBattle}>March to Campaign</button></div>
      </div>
    </Modal>
