...
import { CAMPAIGNS, COPY_THRESHOLDS, GOLD_UPGRADE_COSTS, PEOPLE, RARITIES, START_META, TECHS, UNITS } from './data'
...
function PersonThumb({id,meta,onClick}){
  const p=PEOPLE[id], art=ASSETS.people[id],level=meta.characterLevels[id]||1,copies=meta.characterCopies[id]||0
  return <button className="person-thumb" onClick={onClick} style={{'--rarity':RARITIES[p.rarity].color}}>
    {art?<img src={art.front} alt={p.name}/>:<div className="emblem-thumb"><span>{p.icon}</span></div>}
...
function UnitCard({id}){const u=UNITS[id];return <div className="unit-card"><div className="unit-icon">{u.role.includes('MOUNTED')?'🐎':u.role.includes('RANGED')||u.role.includes('FIREARM')?'🏹':u.role.includes('SPEAR')?'⚑':'⚔'}</div><h4>{u.name}</h4><span className="unit-role">{u.role}</span><div className="unit-stats"><UnitStat label="Health" value={u.health}/><UnitStat label="Armor" value={u.armor}/><UnitStat label="Damage" value={u.damage}/><UnitStat label="Atk Spd" value={u.attackSpeed}/><UnitStat label="Range" value={u.range}/></div><p>{u.desc}</p></div>}
...
export default function App(){
  ...
  const ownedLeaders=ownedPeople.filter(id=>PEOPLE[id].type==='leader')
  const ownedGenerals=ownedPeople.filter(id=>PEOPLE[id].type==='general')
  ...
  function patchMeta(fn){setMeta(m=>{const next=typeof fn==='function'?fn(clone(m)):{...m,...fn};return next})}
  function notify(s){setToast(s)}

  function selectCampaign(id){if(!meta.unlockedCampaigns.includes(id))return;setSelectedCampaign(id);patchMeta(m=>{m.loadout.tech=m.loadout.tech.filter(t=>!t||TECHS[t].tier<=CAMPAIGNS[id].maxTechTier).slice(0,effectiveTechSlots);while(m.loadout.tech.length<effectiveTechSlots)m.loadout.tech.push(null);return m})}
  ...
  function openMuster(){const pool=availableMusterUnits();setMuster(Array.from({length:startingCapacity},(_,i)=>pool[Math.min(i,pool.length-1)]||'warrior'));setMusterOpen(true)}
  function cycleMuster(slot,id){setMuster(a=>a.map((x,i)=>i===slot?id:x))}
  function launchBattle(){setMusterOpen(false);setBattle({campaignId:selectedCampaign,loadout:clone(meta.loadout),startingRoster:[...muster],startCapacity:startingCapacity});setScreen('battle')}
  ...
  function finishRun(info){
    patchMeta(m=>{
      ...
      return m
    });setBattle(null);setScreen('campaign');notify(info.victory?'Campaign conquered. New history unlocked.':'Run resources banked.')
  }
  ...
  function upgradePerson(id){const p=PEOPLE[id],level=meta.characterLevels[id]||1,max=RARITIES[p.rarity].maxLevel;if(level>=max)return;const need=COPY_THRESHOLDS[level],cost=GOLD_UPGRADE_COSTS[level];if((meta.characterCopies[id]||0)<need||meta.gold<cost)return
    patchMeta(m=>{m.gold-=cost;m.characterLevels[id]=level+1;const sig=p.signatureUnit;if(sig&&!m.discoveredUnits.includes(sig))m.discoveredUnits.push(sig);return m});notify(`${p.name} reached Level ${level+1}.`)
  }
  ...
  return <div className="app-shell">
    <header className="topbar"><img src={ASSETS.logo} className="main-logo" alt="March of Epochs"/><div className="resources"><span>◉ <b>{Math.floor(meta.gold)}</b> Gold</span><span>✦ <b>{Math.floor(meta.tp)}</b> Tech</span></div></header>

    <main>
      {screen==='campaign'&&<section className="screen-panel">
        <div className="hero-banner"><span>BUILD A CIVILIZATION. LOSE IT. RETURN STRONGER.</span><h1>Campaign</h1><p>Select a historical chapter, choose your people and technologies, then muster only the units your build makes available.</p></div>
        <div className="content-pad">
          <div className="section-head"><h3>Campaigns</h3><span>15 battles · milestones after 5, 10 and 14</span></div>
          <div className="campaign-grid">{Object.values(CAMPAIGNS).map(c=>{const unlocked=meta.unlockedCampaigns.includes(c.id),selected=c.id===selectedCampaign;return <button key={c.id} className={`campaign-card ${selected?'selected':''} ${unlocked?'':'locked'}`} onClick={()=>selectCampaign(c.id)}><div className={`campaign-scene ${c.id}`}></div><div><b>{c.name}</b><span>{c.era}</span><p>{c.desc}</p><small>{unlocked?(meta.completedCampaigns.includes(c.id)?'✓ Completed':'Available'):'🔒 Complete previous campaign'}</small></div></button>})}</div>
          <div className="section-head"><h3>Run loadout</h3><span>{effectiveTechSlots} technology slots · starting army {startingCapacity}</span></div>
          <div className="loadout-grid">
            <button className="loadout-slot person" onClick={()=>openPicker('leader')}><span>LEADER</span>{ASSETS.people[meta.loadout.leader]?<img src={ASSETS.people[meta.loadout.leader].front}/>:<div className="slot-emblem">{leader?.icon}</div>}<b>{leader?.name}</b><small>Level {meta.characterLevels[meta.loadout.leader]||1}</small></button>
            <button className="loadout-slot person" onClick={()=>openPicker('general')}><span>GENERAL</span>{ASSETS.people[meta.loadout.general]?<img src={ASSETS.people[meta.loadout.general].front}/>:<div className="slot-emblem">{general?.icon}</div>}<b>{general?.name}</b><small>Level {meta.characterLevels[meta.loadout.general]||1}</small></button>
      ...
      {screen==='collection'&&<section className="screen-panel"><div className="hero-banner"><span>THE CHRONICLE</span><h1>Collection</h1><p>Only people you have actually discovered are visible. Premium character art is stored as real front/back game assets.</p></div><div className="content-pad"><div className="subtabs">{[['people','People'],['technology','Technologies'],['units','Units']].map(([id,n])=><button key={id} className={collectionTab===id?'active':''} onClick={()=>setCollectionTab(id)}>{n}</button>)}</div>
        {collectionTab==='people'&&<><div className="collection-meta"><span>{ownedPeople.length} people discovered</span><span>Tap a card to inspect and flip it</span></div><div className="people-grid">{ownedPeople.map(id=><div key={id} className="collection-person" onClick={()=>setDetailPerson(id)}>{ASSETS.people[id]?<img src={ASSETS.people[id].front} alt={PEOPLE[id].name}/>:<div className="generic-collection"><span>{PEOPLE[id].icon}</span><b>{PEOPLE[id].name}</b><small>{PEOPLE[id].rarity}</small></div>}<div className="collection-person-meta"><b>Lv {meta.characterLevels[id]||1}</b><span>{meta.characterCopies[id]||0} cards</span></div></div>)}</div></>}
    ...
    <Modal open={musterOpen} onClose={()=>setMusterOpen(false)} title={`Muster starting army · ${startingCapacity} squads`} wide><div className="muster-columns"><div className="muster-slots">{muster.map((id,i)=><div key={i}><span>Squad {i+1}</span><UnitCard id={id}/></div>)}</div><div><h3>Available from this build</h3><div className="muster-pool">{availableMusterUnits().map(id=><div key={id}><b>{UNITS[id].name}</b><span>{UNITS[id].role}</span></div>)}</div><p className="micro-copy">Tap a squad slot below, then choose a unit.</p><div className="slot-selector">{muster.map((id,i)=><button key={i} onClick={()=>setPicker({type:'muster',index:i})}>#{i+1} {UNITS[id].name}</button>)}</div><button className="primary big" onClick={launchBattle}>March to Campaign</button></div></div></Modal>

    <Modal open={picker?.type==='muster'} onClose={()=>setPicker(null)} title={`Choose unit for squad ${(picker?.index??0)+1}`} wide><div className="units-grid picker">{availableMusterUnits().map(id=><button key={id} className="unit-pick" onClick={()=>{cycleMuster(picker.index,id);setPicker(null)}}><UnitCard id={id}/></button>)}</div></Modal>

    <Modal open={!!detailPerson} onClose={()=>setDetailPerson(null)} title={detailPerson?PEOPLE[detailPerson].name:''} wide>{detailPerson&&<div className="person-detail-layout"><CardFlip id={detailPerson} meta={meta} onUpgrade={upgradePerson}/><div className="person-details"><h3>{PEOPLE[detailPerson].rarity} {PEOPLE[detailPerson].type}</h3><p>{PEOPLE[detailPerson].bio}</p><h4>Gameplay level path</h4><ol>{PEOPLE[detailPerson].levels.slice(0,RARITIES[PEOPLE[detailPerson].rarity].maxLevel).map((x,i)=><li key={i} className={i<(meta.characterLevels[detailPerson]||1)?'active':''}>{x}</li>)}</ol>{PEOPLE[detailPerson].signatureUnit&&<div className="signature-callout"><b>Signature unit</b><span>{UNITS[PEOPLE[detailPerson].signatureUnit]?.name}</span></div>}<small className="micro-copy">The premium back artwork is illustrative UI art. The mechanics listed here are the authoritative prototype values.</small></div></div>}</Modal>

    <Modal open={!!pack} onClose={()=>setPack(null)} title={pack?.name||'Historical Pack'} wide>{pack&&pack.stage==='sealed'?<div className="sealed-pack" onClick={revealPack}><img src={ASSETS.logo}/><div className="wax-seal">ME</div><h3>Tap to open</h3><p>Characters remain hidden until the cards turn over.</p></div>:pack&&<div><div className="pack-results">{pack.results.map((result,i)=>{const id=result.id;return <div key={`${id}-${i}`} className="pack-result" style={{animationDelay:`${i*.14}s`}}>{ASSETS.people[id]?<img src={ASSETS.people[id].front}/>:<div className="generic-pack"><span>{PEOPLE[id].icon}</span><b>{PEOPLE[id].name}</b><small>{PEOPLE[id].rarity}</small></div>}<b>{PEOPLE[id].name}</b><span>{result.isNew?'NEW DISCOVERY':'DUPLICATE'}</span></div>})}</div><div className="pack-bonus">＋ ◉ {pack.bonus} Gold</div><button className="primary big" onClick={()=>setPack(null)}>Add to Collection</button></div>}</Modal>
