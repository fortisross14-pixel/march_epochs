  ...
  return <div className="app-shell mobile-first-shell menu-shell" onClickCapture={event=>{const button=event.target.closest('button');if(button&&!button.disabled)emitFeedback(button.classList.contains('primary')?'primary':'tap')}}>
    ...
    <main key={screen}>
      ...
      {screen==='artifacts'&&<section className="screen-panel mobile-screen"><div className="simple-screen-title"><span>PERMANENT EQUIPMENT</span><h1>Artifacts</h1><p>Equip your armies. Evolve your collection.</p></div><div className="content-pad"><div className="subtabs">{[['army','Army Artifacts'],['relics',features.relics?'Hero Relics':'Relics Locked'],['store','Pack Store']].map(([id,n])=><button key={id} aria-pressed={artifactTab===id} className={artifactTab===id?'active':''} onClick={()=>{if(id==='relics'&&!features.relics)notify('Hero Relics unlock after winning battle 2-5.','invalid');else setArtifactTab(id)}}>{n}</button>)}</div>
        ...
        {artifactTab==='store'&&<div className="pack-store"><div className="subtabs inner-store-tabs"><button aria-pressed={artifactStoreCategory==='artifacts'} className={artifactStoreCategory==='artifacts'?'active':''} onClick={()=>setArtifactStoreCategory('artifacts')}>Artifact Packs</button><button aria-pressed={artifactStoreCategory==='heroes'} className={artifactStoreCategory==='heroes'?'active':''} onClick={()=>features.people?setArtifactStoreCategory('heroes'):notify('Hero packs unlock with People after battle 1-5.','invalid')}>{features.people?'Hero Packs':'Hero Packs Locked'}</button></div>{artifactStoreCategory==='artifacts'?<div className="pack-grid mobile-packs"><button className="pack-box artifact-pack" disabled={meta.gold<packSpec(meta,'basic','artifact').cost} onClick={()=>openPack('basic','artifact')}><img className="pack-chest-art" src={ASSETS.ui.chestBronze} alt=""/><h3>Supply Artifact Pack</h3><p>1 random early Army Artifact.</p><b>◉ {packSpec(meta,'basic','artifact').cost} Gold</b>{meta.gold<packSpec(meta,'basic','artifact').cost&&<small className="feedback-note">Need {packSpec(meta,'basic','artifact').cost-meta.gold} more Gold</small>}</button><button className="pack-box artifact-pack great" disabled={meta.gold<packSpec(meta,'advanced','artifact').cost} onClick={()=>openPack('advanced','artifact')}><img className="pack-chest-art" src={ASSETS.ui.chestPurple} alt=""/><h3>Advanced Artifact Pack</h3><p>2 useful Army Artifacts at a bundle price.</p><b>◉ {packSpec(meta,'advanced','artifact').cost} Gold</b>{meta.gold<packSpec(meta,'advanced','artifact').cost&&<small className="feedback-note">Need {packSpec(meta,'advanced','artifact').cost-meta.gold} more Gold</small>}</button>{features.relics&&<div className="store-note"><b>Earn Hero Relics</b><p>Claim Hero Relics from achievements, then equip them to your People.</p></div>}</div>:<div className="pack-grid mobile-packs"><button className="pack-box" disabled={meta.gold<packSpec(meta,'standard','people').cost} onClick={()=>openPack('standard','people')}><img className="pack-chest-art" src={ASSETS.ui.chestBlue} alt=""/><h3>Historical Pack</h3><p>2 random Leader/General cards + a small Gold return.</p><b>◉ {packSpec(meta,'standard','people').cost} Gold</b>{meta.gold<packSpec(meta,'standard','people').cost&&<small className="feedback-note">Need {packSpec(meta,'standard','people').cost-meta.gold} more Gold</small>}</button><button className="pack-box great" disabled={meta.gold<packSpec(meta,'great','people').cost} onClick={()=>openPack('great','people')}><img className="pack-chest-art" src={ASSETS.ui.chestGold} alt=""/><h3>Great Chronicle</h3><p>3 People cards, including at least one Rare or better.</p><b>◉ {packSpec(meta,'great','people').cost} Gold</b>{meta.gold<packSpec(meta,'great','people').cost&&<small className="feedback-note">Need {packSpec(meta,'great','people').cost-meta.gold} more Gold</small>}</button></div>}</div>}
