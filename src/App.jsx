...
function TechNode({id,meta,onBuy}){
  const t=TECHS[id],owned=meta.ownedTech.includes(id),available=t.prereq.every(p=>meta.ownedTech.includes(p)),canBuy=canResearch(meta,id)&&meta.tp>=t.cost,card=ASSETS.techCards?.[id]
  return <button className={`tech-node ${card?'illustrated-tech':''} ${owned?'owned':''} ${available?'available':'locked'}`} disabled={!canBuy} onClick={()=>canBuy&&onBuy(id)}>
    <div className={`tech-icon art ${card?'full-card':''}`}>{card?<img src={card} alt={t.name}/>:ASSETS.tech?.[id]?<img src={ASSETS.tech[id]} alt={t.name}/>:t.icon}</div><div className="tech-copy"><b>{t.name}</b><span>Tier {t.tier}{owned?' · OWNED':!available?' · LOCKED':<> · <img className="inline-currency-icon" src={ASSETS.ui.tp} alt="TP"/> {t.cost}</>}</span><small>{t.desc}</small><em className="research-status">{owned?'Researched':!available?`Requires ${t.prereq.filter(p=>!meta.ownedTech.includes(p)).map(p=>TECHS[p].name).join(', ')}`:canBuy?'Research':`Need ${t.cost} TP`}</em></div>
  </button>
}
...
function ArtifactTileVisual({item,level,available,equipped,canUpgrade,onClick}){
  return <button className={`artifact-tile menu-artifact-card rarity-${item.rarity.toLowerCase()}`} aria-label={`${item.name}, level ${level}, ${available} available`} style={{'--rarity':RARITIES[item.rarity].color}} onClick={onClick}>
    <div className="artifact-card-top"><span>Lv {level}</span><small>{item.rarity}</small></div><ArtifactArtwork id={item.id}/><div className="artifact-card-caption"><b>{item.name}</b><small>{available} available{equipped?' / '+equipped+' equipped':''}</small>{canUpgrade&&<span>Upgrade ready</span>}</div>
  </button>
}
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    if(newlyUnlocked.length)setUnlockNotice(newlyUnlocked)
  },[meta.featureUnlocks])

  useEffect(()=>saveMeta(meta),[meta])
  useEffect(()=>{if(screen!=='battle')window.scrollTo(0,0)},[screen])
  ...
  function chooseScreen(id){
    ...
    if(id==='technology'&&!features.technology){notify('Technology unlocks after winning battle 1-2.');return}
    setScreen(id)
  }
  ...
  function buyTech(id){const t=TECHS[id];if(!canResearch(meta,id)||meta.tp<t.cost)return;patchMeta(m=>{researchTech(m,id);return m});notify(t.name+' researched.')}
  ...
  function openPack(kind,packType='people'){
    if(packType==='people'&&!features.people)return
    const spec=packSpec(meta,kind,packType);if(meta.gold<spec.cost||!spec.pool.length){notify('Not enough Gold.');return}
  ...
  function revealPack(){setPack(p=>p?{...p,stage:'revealed'}:p)}
  function upgradePerson(id){patchMeta(m=>{upgradePersonRule(m,id);return m})}
  ...
  function upgradeUnit(id){
    ...
    patchMeta(m=>{trainUnit(m,id);return m});notify(`${UNITS[id].name} reached Level ${level+1}.`)
  }

  function buyArtifact(id){const item=ITEMS[id],cost=supplyPrice(meta,item.price);if(!artifactSupply(meta).includes(id)||meta.gold<cost)return;patchMeta(m=>{buySupply(m,id);return m});notify(item.name+' acquired.')}
  function evolveArtifact(id){patchMeta(m=>{evolveArtifactRule(m,id);return m})}
  ...
  function equipItem(itemId,targetId){if(availableItemCopies(itemId)<1)return;patchMeta(m=>{equipArtifact(m,itemId,targetId);return m});setEquipPicker(null)}
  ...
  return <div className="app-shell mobile-first-shell menu-shell">
    ...
    <main key={screen}>
      ...
      {screen==='artifacts'&&<section className="screen-panel mobile-screen"><div className="simple-screen-title"><span>PERMANENT EQUIPMENT</span><h1>Artifacts</h1><p>Equip your armies. Evolve your collection.</p></div><div className="content-pad"><div className="subtabs">{[['army','Army Artifacts'],['relics',features.relics?'Hero Relics':'Relics Locked'],['store','Pack Store']].map(([id,n])=><button key={id} className={artifactTab===id?'active':''} onClick={()=>{if(id==='relics'&&!features.relics)notify('Hero Relics unlock after winning battle 2-5.');else setArtifactTab(id)}}>{n}</button>)}</div>
        {artifactTab==='army'&&<><div className="section-head"><h3>Your Army Artifacts</h3><span>{Object.values(ITEMS).filter(i=>i.slot==='unit'&&(meta.inventory[i.id]||0)>0).length} collected</span></div>{Object.values(ITEMS).filter(i=>i.slot==='unit'&&(meta.inventory[i.id]||0)>0).length?<div className="artifact-tile-grid">{Object.values(ITEMS).filter(i=>i.slot==='unit'&&(meta.inventory[i.id]||0)>0).map(item=>{const copies=meta.inventory[item.id]||0,available=availableItemCopies(item.id),equipped=equippedItemCount(item.id),level=meta.artifactLevels[item.id]||1,max=level>=item.maxLevel,need=max?0:ARTIFACT_COPY_THRESHOLDS[level],canUpgrade=!max&&copies>=need&&meta.gold>=ARTIFACT_WORKSHOP_GOLD[level];return <ArtifactTileVisual key={item.id} item={item} level={level} available={available} equipped={equipped} canUpgrade={canUpgrade} onClick={()=>setDetailArtifact(item.id)}/>})}</div>:<div className="empty-state"><b>No Army Artifacts collected</b><span>Choose supplies or open a pack in the Pack Store. Equipped copies stay in your collection for Workshop upgrades.</span><button className="primary" onClick={()=>setArtifactTab('store')}>Open Pack Store</button></div>}</>}
        {artifactTab==='relics'&&features.relics&&<><div className="section-head"><h3>Your Hero Relics</h3><span>{Object.values(ITEMS).filter(i=>i.slot==='hero'&&availableItemCopies(i.id)>0).length} available in inventory</span></div>{Object.values(ITEMS).filter(i=>i.slot==='hero'&&availableItemCopies(i.id)>0).length?<div className="artifact-tile-grid">{Object.values(ITEMS).filter(i=>i.slot==='hero'&&availableItemCopies(i.id)>0).map(item=>{const copies=meta.inventory[item.id]||0,available=availableItemCopies(item.id),equipped=equippedItemCount(item.id),level=meta.artifactLevels[item.id]||1,max=level>=item.maxLevel,need=max?0:ARTIFACT_COPY_THRESHOLDS[level],canUpgrade=!max&&copies>=need&&meta.gold>=ARTIFACT_WORKSHOP_GOLD[level];return <ArtifactTileVisual key={item.id} item={item} level={level} available={available} equipped={equipped} canUpgrade={canUpgrade} onClick={()=>setDetailArtifact(item.id)}/>})}</div>:<div className="empty-state"><b>No Hero Relics available</b><span>Relic packs appear in the store once this feature is unlocked. Equipped Relics are hidden from this inventory list.</span></div>}</>}
        {artifactTab==='store'&&<div className="pack-store"><div className="subtabs inner-store-tabs"><button className={artifactStoreCategory==='artifacts'?'active':''} onClick={()=>setArtifactStoreCategory('artifacts')}>Artifact Packs</button><button className={artifactStoreCategory==='heroes'?'active':''} onClick={()=>features.people?setArtifactStoreCategory('heroes'):notify('Hero packs unlock with People after battle 1-5.')}>{features.people?'Hero Packs':'Hero Packs Locked'}</button></div>{artifactStoreCategory==='artifacts'?<div className="pack-grid mobile-packs"><button className="pack-box artifact-pack" disabled={meta.gold<packSpec(meta,'basic','artifact').cost} onClick={()=>openPack('basic','artifact')}><img className="pack-chest-art" src={ASSETS.ui.chestBronze} alt=""/><h3>Supply Artifact Pack</h3><p>1 random early Army Artifact.</p><b>◉ {packSpec(meta,'basic','artifact').cost} Gold</b></button><button className="pack-box artifact-pack great" disabled={meta.gold<packSpec(meta,'advanced','artifact').cost} onClick={()=>openPack('advanced','artifact')}><img className="pack-chest-art" src={ASSETS.ui.chestPurple} alt=""/><h3>Advanced Artifact Pack</h3><p>2 useful Army Artifacts at a bundle price.</p><b>◉ {packSpec(meta,'advanced','artifact').cost} Gold</b></button>{features.relics&&<div className="store-note"><b>Earn Hero Relics</b><p>Claim Hero Relics from achievements, then equip them to your People.</p></div>}</div>:<div className="pack-grid mobile-packs"><button className="pack-box" disabled={meta.gold<packSpec(meta,'standard','people').cost} onClick={()=>openPack('standard','people')}><img className="pack-chest-art" src={ASSETS.ui.chestBlue} alt=""/><h3>Historical Pack</h3><p>2 random Leader/General cards + a small Gold return.</p><b>◉ {packSpec(meta,'standard','people').cost} Gold</b></button><button className="pack-box great" disabled={meta.gold<packSpec(meta,'great','people').cost} onClick={()=>openPack('great','people')}><img className="pack-chest-art" src={ASSETS.ui.chestGold} alt=""/><h3>Great Chronicle</h3><p>3 People cards, including at least one Rare or better.</p><b>◉ {packSpec(meta,'great','people').cost} Gold</b></button></div>}</div>}
        {artifactTab==='store'&&artifactStoreCategory==='artifacts'&&<><div className="section-head"><h3>Choose your supplies</h3><span>Guaranteed equipment</span></div><div className="artifact-tile-grid">{artifactSupply(meta).map(id=><button className="artifact-tile menu-artifact-card supply-card" key={id} disabled={meta.gold<supplyPrice(meta,ITEMS[id].price)} onClick={()=>buyArtifact(id)}><ArtifactArtwork id={id}/><b>{ITEMS[id].name}</b><small>{effectSummary(ITEMS[id],meta.artifactLevels[id]||1)}</small><span>{supplyPrice(meta,ITEMS[id].price)} Gold</span></button>)}</div></>}
    ...
    <Modal menu flow back open={campaignPrepOpen&&!picker} onClose={closeCampaignPrep} title="Campaign Loadout" wide>
      <div className="campaign-prep-layout">
        ...
        {features.technology?<div className="loadout-grid technology-loadout prep-grid">{Array.from({length:effectiveTechSlots},(_,i)=>{const id=meta.loadout.tech[i];return <button key={i} className="loadout-slot tech prep-card" onClick={()=>openPicker('tech',i)}><span>SLOT {i+1}</span><div className="tech-big art">{id&&ASSETS.tech?.[id]?<img src={ASSETS.tech[id]} alt=""/>:'＋'}</div><b>{id?TECHS[id].name:'Choose technology'}</b><small>{id?'Tap to change':'Optional'}</small></button>})}</div>:<div className="feature-lock-banner inline"><img src={ASSETS.ui.lock} alt="Locked"/><div><b>Research & Technology</b><span>Win battle 1-2 to unlock.</span></div></div>}
