...
function TechNode({id,meta,onBuy}){
  const t=TECHS[id],owned=meta.ownedTech.includes(id),available=t.prereq.every(p=>meta.ownedTech.includes(p)),canBuy=!owned&&available&&meta.tp>=t.cost,card=ASSETS.techCards?.[id]
  return <button className={`tech-node ${card?'illustrated-tech':''} ${owned?'owned':''} ${available?'available':'locked'}`} disabled={!owned&&!available} onClick={()=>canBuy&&onBuy(id)}>
    <div className={`tech-icon art ${card?'full-card':''}`}>{card?<img src={card} alt={t.name}/>:ASSETS.tech?.[id]?<img src={ASSETS.tech[id]} alt={t.name}/>:t.icon}</div><div className="tech-copy"><b>{t.name}</b><span>Tier {t.tier}{owned?' · OWNED':!available?' · LOCKED':<> · <img className="inline-currency-icon" src={ASSETS.ui.tp} alt="TP"/> {t.cost}</>}</span><small>{t.desc}</small></div>
  </button>
}
...
function ArtifactTileVisual({item,level,available,equipped,canUpgrade,onClick}){
  const card=ASSETS.artifactCards?.[item.id]
  return <button className={`artifact-tile ${card?'illustrated-artifact':''} rarity-${item.rarity.toLowerCase()}`} onClick={onClick}>
    {card?<img className="artifact-illustrated-card" src={card} alt={item.name}/>:<><img className="artifact-frame" src={itemFrame(item.rarity)} alt=""/><div className="artifact-corner family">{itemFamilyIcon(item)}</div><div className="artifact-art-wrap">{ASSETS.items?.[item.id]?<img src={ASSETS.items[item.id]} alt={item.name}/>:<span>✦</span>}</div><div className="artifact-caption"><b>{item.name}</b><small>{item.rarity}{equipped?` · ${equipped} eq.`:''}</small></div></>}
    <div className="artifact-corner level">Lv.{level}</div><div className="artifact-count">{available}</div>{canUpgrade&&<img className="artifact-upgrade-ready" src={ASSETS.ui.upgradeReady} alt="Ready to upgrade"/>}
  </button>
}
...
export default function App(){
  ...
  function openPack(kind,packType='people'){
    ...
    const spec=kind==='advanced'
      ?{cost:95,count:2,name:'Advanced Artifact Pack',pool:['leatherLamellar','stoneAxeGrip','boneCharm','bronzeArmor','bronzeEdges','spearheads','edgedProjectiles','slingPouch']}
      :{cost:45,count:1,name:'Supply Artifact Pack',pool:['leatherLamellar','stoneAxeGrip','boneCharm','bronzeArmor','bronzeEdges','spearheads','slingPouch']}
    if(meta.gold<spec.cost){notify('Not enough Gold.');return}
    patchMeta(m=>{m.gold-=spec.cost;m.stats.packsOpened=(m.stats.packsOpened||0)+1;return m})
  ...
  return <div className="app-shell mobile-first-shell">
    ...
    <main>
      {screen==='campaign'&&<section className="screen-panel mobile-screen">
        ...
        <div className="content-pad">
          ...
          <div className="campaign-shell-summary">
            ...
            <div className="battle-shell-list compact-nodes">{campaign.stages.map((s,i)=>{const status=i<(currentCampaignStats.best||0)?'cleared':i===currentCampaignStats.best?'next':'locked';return <div key={s.name} title={s.name} className={status}><b>{i+1}</b></div>})}</div>
      ...
      {screen==='artifacts'&&<section className="screen-panel mobile-screen"><div className="simple-screen-title"><span>PERMANENT EQUIPMENT</span><h1>Artifacts</h1><p>Artifacts now come from packs. Equip up to two Army Artifacts on each unit type; Hero Relics unlock later.</p></div><div className="content-pad"><div className="subtabs">{[['army','Army Artifacts'],['relics',features.relics?'Hero Relics':'Relics Locked'],['store','Pack Store']].map(([id,n])=><button key={id} className={artifactTab===id?'active':''} onClick={()=>{if(id==='relics'&&!features.relics)notify('Hero Relics unlock after winning battle 2-5.');else setArtifactTab(id)}}>{n}</button>)}</div>
        {artifactTab==='army'&&<><div className="section-head"><h3>Your Army Artifacts</h3><span>{Object.values(ITEMS).filter(i=>i.slot==='unit'&&availableItemCopies(i.id)>0).length} available in inventory</span></div>{Object.values(ITEMS).filter(i=>i.slot==='unit'&&availableItemCopies(i.id)>0).length?<div className="artifact-tile-grid">{Object.values(ITEMS).filter(i=>i.slot==='unit'&&availableItemCopies(i.id)>0).map(item=>{const copies=meta.inventory[item.id]||0,available=availableItemCopies(item.id),equipped=equippedItemCount(item.id),level=meta.artifactLevels[item.id]||1,max=level>=item.maxLevel,need=max?0:ARTIFACT_COPY_THRESHOLDS[level],canUpgrade=!max&&copies>=need&&meta.gold>=ARTIFACT_WORKSHOP_GOLD[level];return <ArtifactTileVisual key={item.id} item={item} level={level} available={available} equipped={equipped} canUpgrade={canUpgrade} onClick={()=>setDetailArtifact(item.id)}/>})}</div>:<div className="empty-state"><b>No Army Artifacts available</b><span>Open a Supply Artifact Pack in the Pack Store. Equipped Artifacts are hidden from this inventory list until you remove them from a unit.</span><button className="primary" onClick={()=>setArtifactTab('store')}>Open Pack Store</button></div>}</>}
        ...
        {artifactTab==='store'&&<div className="pack-store"><div className="subtabs inner-store-tabs"><button className={artifactStoreCategory==='artifacts'?'active':''} onClick={()=>setArtifactStoreCategory('artifacts')}>Artifact Packs</button><button className={artifactStoreCategory==='heroes'?'active':''} onClick={()=>features.people?setArtifactStoreCategory('heroes'):notify('Hero packs unlock with People after battle 1-5.')}>{features.people?'Hero Packs':'Hero Packs Locked'}</button></div>{artifactStoreCategory==='artifacts'?<div className="pack-grid mobile-packs"><button className="pack-box artifact-pack" onClick={()=>openPack('basic','artifact')}><img className="pack-chest-art" src={ASSETS.ui.chestBronze} alt=""/><h3>Supply Artifact Pack</h3><p>1 random early Army Artifact. Best first purchase for a new account.</p><b>◉ 45 Gold</b></button><button className="pack-box artifact-pack great" onClick={()=>openPack('advanced','artifact')}><img className="pack-chest-art" src={ASSETS.ui.chestPurple} alt=""/><h3>Advanced Artifact Pack</h3><p>2 Army Artifacts with a broader chance of melee, spear and ranged gear.</p><b>◉ 95 Gold</b></button>{features.relics&&<div className="store-note"><b>Relic packs coming next</b><p>Hero Relics are unlocked; this build keeps their first pool achievement-driven while we expand the relic library.</p></div>}</div>:<div className="pack-grid mobile-packs"><button className="pack-box" onClick={()=>openPack('standard','people')}><img className="pack-chest-art" src={ASSETS.ui.chestBlue} alt=""/><h3>Historical Pack</h3><p>2 random Leader/General cards + a small Gold return.</p><b>◉ 100 Gold</b></button><button className="pack-box great" onClick={()=>openPack('great','people')}><img className="pack-chest-art" src={ASSETS.ui.chestGold} alt=""/><h3>Great Chronicle</h3><p>3 people with improved odds of Rare+ discoveries.</p><b>◉ 220 Gold</b></button></div>}</div>}
    ...
    <Modal open={campaignPrepOpen} onClose={closeCampaignPrep} title={`Start Campaign · Step 1 of 2`} wide>
      <div className="campaign-prep-layout">
        ...
        <div className="loadout-grid command-loadout prep-grid">
          {features.people?<><button className="loadout-slot person prep-card" onClick={()=>openPicker('leader')}><span>LEADER</span><MiniCardArt id={meta.loadout.leader} meta={meta}/><b>{leader?.name||'Select Leader'}</b><small>Level {meta.characterLevels[meta.loadout.leader]||1}</small></button><button className="loadout-slot person prep-card" onClick={()=>openPicker('general')}><span>GENERAL</span><MiniCardArt id={meta.loadout.general} meta={meta}/><b>{general?.name||'Select General'}</b><small>Level {meta.characterLevels[meta.loadout.general]||1}</small></button></>:<><div className="loadout-slot feature-locked prep-card"><span>LEADER</span><strong className="lock-art"><img src={ASSETS.ui.lock} alt="Locked"/></strong><b>People locked</b><small>Win battle 1-5 to unlock.</small></div><div className="loadout-slot feature-locked prep-card"><span>GENERAL</span><strong className="lock-art"><img src={ASSETS.ui.lock} alt="Locked"/></strong><b>People locked</b><small>Win battle 1-5 to unlock.</small></div></>}
        </div>
    ...
    <Modal open={!!pack} onClose={()=>setPack(null)} title={pack?.name||'Pack'} wide>{pack&&pack.stage==='sealed'?<div className="sealed-pack illustrated" onClick={revealPack}><img className="sealed-chest" src={pack.packType==='artifact'?(pack.kind==='advanced'?ASSETS.ui.chestPurple:ASSETS.ui.chestBronze):(pack.kind==='great'?ASSETS.ui.chestGold:ASSETS.ui.chestBlue)} alt="Pack"/><h3>Tap to open</h3><p>{pack.packType==='artifact'?'Artifacts are revealed when the chest opens.':'People remain unknown until revealed.'}</p></div>:pack&&pack.packType==='artifact'?<div><div className="pack-results artifact-results">{pack.results.map((result,i)=>{const item=ITEMS[result.id];return <div key={`${result.id}-${i}`} className="pack-result artifact-pack-result"><div className="artifact-art pack-card-art">{ASSETS.artifactCards?.[result.id]?<img src={ASSETS.artifactCards[result.id]} alt={item.name}/>:ASSETS.items?.[result.id]?<img src={ASSETS.items[result.id]} alt={item.name}/>:<span>✦</span>}</div><b>{item.name}</b><span>{result.isNew?'NEW ARTIFACT':'DUPLICATE COPY'}</span><small>{item.rarity} · {effectSummary(item,meta.artifactLevels[result.id]||1)}</small></div>})}</div><button className="primary big" onClick={()=>setPack(null)}>Add to Inventory</button></div>:pack&&<div><div className="pack-results slim">{pack.results.map((result,i)=><div key={`${result.id}-${i}`} className="pack-result"><MiniCardArt id={result.id} meta={meta}/><b>{PEOPLE[result.id].name}</b><span>{result.isNew?'NEW DISCOVERY':'DUPLICATE'}</span></div>)}</div><div className="pack-bonus">＋ ◉ {pack.bonus} Gold</div><button className="primary big" onClick={()=>setPack(null)}>Add to Collection</button></div>}</Modal>
    ...
    <Modal open={achievementsOpen} onClose={()=>setAchievementsOpen(false)} title="Achievements" wide>{(()=>{const all=Object.values(ACHIEVEMENTS),status=a=>{const value=meta.stats?.[a.stat]||0,claimed=meta.claimedAchievements.includes(a.id);return{value,claimed,ready:value>=a.target&&!claimed}},card=a=>{const s=status(a);return <div key={a.id} className={`achievement-card ${s.ready?'ready':''}`}><div className="achievement-icon"><img src={ASSETS.ui.trophy} alt=""/></div><div><h3>{a.name}</h3><p>{a.desc}</p><div className="achievement-progress"><i><em style={{width:`${Math.min(100,s.value/a.target*100)}%`}}/></i><span>{Math.min(s.value,a.target)}/{a.target}</span></div><small>Reward: ◉ {a.reward}{a.item?` + ${ITEMS[a.item].name}`:''}</small></div><button disabled={!s.ready} onClick={()=>claimAchievement(a.id)}>{s.ready?'Claim':'Go'}</button></div>},ready=all.filter(a=>status(a).ready).sort((a,b)=>((a.order||0)-(b.order||0))||a.name.localeCompare(b.name)),families=[...new Set(all.map(a=>a.family||'Other'))];return <div className="achievement-modal-content">{ready.length>0&&<><div className="achievement-family-title"><b>Ready to Claim</b><span>Completed achievements disappear from the list after claiming.</span></div><div className="achievement-grid">{ready.map(card)}</div></>}{families.map(f=>{const items=all.filter(a=>(a.family||'Other')===f&&!status(a).claimed&&!status(a).ready).sort((a,b)=>(a.order||0)-(b.order||0));if(!items.length)return null;return <div key={f} className="achievement-family"><div className="achievement-family-title"><b>{f}</b><span>Progression chain</span></div><div className="achievement-grid">{items.map(card)}</div></div>})}</div>})()}</Modal>
